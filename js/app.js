...
if (calContainer) {
    const habitCalController = buildUnifiedCalendar(calContainer, {
      initialDate: new Date(currentHabitDate + "T00:00:00"),
      getCellInfo: (date) => {
        const dStr = dateKeyOf(date);
        const isSelected = dStr === currentHabitDate;
...
function getGymCoverageForDate(date, days) {
  ...
  if (isFuture) return { bg: "#eef2ff", color: "#4338ca", icons };
  return allDone
    ? { bg: "#dcfce7", color: "#15803d", icons }
      ...
      ` : days.slice().sort((a, b) => a.date.localeCompare(b.date)).map(day => {
        ...
        return `
          <div class="gym-day-card" data-day-id="${day.id}" style="
            ...
            padding:1.1rem;margin-bottom:0.8rem;cursor:pointer;
...
function renderGymCreateDay() {
  ...
  return `
    <div style="width:100%;">
      ...
      <h3 style="font-size:1rem;font-weight:700;margin-bottom:0.6rem;">Tipo de día</h3>
...
">${icon("fitness_center")} Gimnasio</button>
      ...
      <button id="saveGymDay" style="
        width:100%;background:#111;color:#fff;border:none;border-radius:14px;
        padding:1rem;font-weight:600;font-size:1rem;cursor:pointer;
      ">Guardar día</button>
    </div>
  `;
}
...
let gymCalController = null;

  function renderCalendarWidget() {
    const container = content.querySelector("#gymCalendarContainer");
    if (!container) return;
    gymCalController = buildUnifiedCalendar(container, {
